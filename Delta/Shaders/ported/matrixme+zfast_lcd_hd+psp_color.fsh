...
//================
const float NATIVE_SCREEN_HEIGHT = 160.0;

//================
void main()
{
    //
    // Note to self: textureCoordinate.xy == 1 / TextureSize.xy
    //
    
    vec2 v_texcoord0 = textureCoordinate; // + 0.000001; // HLSL precision workaround (is this needed???)
    vec2 v_texcoord1 = 1.0 / textureCoordinate.xy; // TextureSize
    
    // Generate grid pattern (in native GBA coordinate space)
    vec2 texcoordInPixels = v_texcoord0.xy * v_texcoord1.xy * NATIVE_SCREEN_HEIGHT * textureCoordinate.y; // v_texcoord1 == TextureSize
    ...
    vec3 screen = pow(texture2D(inputImageTexture, v_texcoord0.xy).rgb, vec3(target_gamma));
